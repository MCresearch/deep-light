...
if option_zernike_corfficient == 1:
    ...
    plt.bar(np.array(range(63)),real_zernike[framnum], color="red",alpha=1,label = "real")
    plt.bar(np.array(range(63)),predict_zernike[framnum], color="blue",alpha=0.5,label = "predict")
    plt.xlabel("Zernike order",fontsize=15)
    plt.ylabel("Zernike coefficient values",fontsize=15)
    plt.xticks([i for i in range(63)],[(i+3) for i in range(63)],size = 5)
    plt.yticks(size=10)
